...
public class Gunner : MonoBehaviour
{
    ...
    void UpdatePlayerPosition()
    {
        ...
        {
            //shoot
            GameObject bulletInstance = Instantiate(bulletPrefab, bulletLaunchPoint.transform.position, Quaternion.EulerAngles(0,0,valueChosen));
            if (scale.x == 1)
            {
                bulletInstance.GetComponent<Rigidbody2D>().velocity = new Vector2(bulletSpeed, 0f);
            }
            else if(scale.x == -1)
            {
                bulletInstance.GetComponent<Rigidbody2D>().velocity = new Vector2(-bulletSpeed, 0f);
